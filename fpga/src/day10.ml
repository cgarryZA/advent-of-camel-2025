...
module Loader = struct
  ...
  let create _ ({ clock; clear; uart_rx; uart_rts } : _ I.t) : _ O.t =
    ...
    (* Pack 8 incoming bytes into one 64-bit word. *)
    let word_in = Util.shift_in ~clock ~clear ~n:8 uart_rx in

    (* IMPORTANT:
       - Use current [word_count] as write address
       - Increment it AFTER a successful word write (i.e. for next cycle)
       This ensures the first word lands at address 0. *)
...
let algo ~clock ~clear ~(read_data : Signal.t array) ~load_finished =
  ...
  let masks =
    Array.init 16 ~f:(fun _ -> Variable.reg spec ~width:64)
  in
  ...
  let state_ok =
    (state_xor &: tgt_mask.value) ==: (target.value &: tgt_mask.value)
  in
  ...
  let limit_val =
    List.init 17 ~f:(fun i ->
      mux2 (k_clip ==: of_int_trunc ~width:8 i)
