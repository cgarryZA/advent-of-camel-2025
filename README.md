...
### Validation & Testing
When you clone this repository, you must populate `/inputs/answers.json` with your own personal Advent of Code answers. This file is treated as the explicit source of truth for validation.

To generate this file:

-Run all implemented days against your full AoC inputs. This produces `/inputs/outputs.json`, containing the results generated by the hardware simulations.

-Manually verify that the values in `outputs.json` match your known-correct AoC solutions.
...
This replaces `inputs/answers.json` with the contents of `inputs/outputs.json`.

---

### Handling long-running inputs

If some days fail unexpectedly (other than Day 7 or Day 10 Part 2), this is likely because your input requires more simulation cycles than the default budget.

To fix this, open:

`run/run_days.ml`

Locate the entry for the failing day:

```ocaml
    | X -> Advent_of_caml.DayX.hierarchical, Advent_of_caml_input_parser.DayX.parse, Some Y
```
...
`Y` is the maximum number of cycles allocated

You can either:

Increase `Y`, or

Set it to `None` to allow the simulation to run without a fixed cycle limit

```ocaml
Some Y   →   None
```

This is purely a simulation constraint and does not indicate a correctness issue in the hardware design.
